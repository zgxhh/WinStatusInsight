...
- **卡顿归因**：把 CPU、内存、进程、资源占用聚合和历史趋势放在同一个视图里，帮助判断卡顿来自浏览器、资源管理器、本地开发服务还是后台进程。
- **本地开发环境治理**：识别正在运行的 Vite、Next、Node、.NET Web API、`.NET watch`、pnpm、bun 等开发服务，并支持对非面板项目执行停止与托盘提醒。
- **低风险系统维护**：只对临时文件、可重建开发缓存和明确可恢复的自启动项提供操作入口；高风险应用数据只做分析提示，不直接删除或迁移。

## 下载

- [安装版 WinStatusInsight-Setup-2.1.0.exe](https://github.com/zgxhh/WinStatusInsight/releases/latest/download/WinStatusInsight-Setup-2.1.0.exe)

WinStatusInsight 从 2.1 起只提供安装版。安装版会写入固定安装目录，并创建开始菜单、桌面快捷方式和卸载入口，更适合长期使用和后续版本更新。若安装在 `D:\Download` 等特殊权限目录后出现瞬退，建议改装到 `D:\Apps\WinStatusInsight` 这类普通目录，或为安装目录授予当前用户完整权限。
...
![资源占用](docs/assets/win-status-insight-app-groups.png)
...
- 将多进程应用聚合到资源占用视图中，例如 Chrome、Edge、Node/Vite、Codex、Windows 资源管理器、Huawei services。
- 汇总进程数、CPU、内存、风险等级、主要进程和处理建议，并支持展开查看具体 PID。
- 支持按综合影响、CPU 优先、内存优先切换排序，不再把 CPU、内存和资源聚合拆成多个重复页面。
...
- 可停止项目支持单独停止或全部停止，系统进程、面板进程和受保护进程会自动跳过，并显示结果摘要。
...
- 支持低风险清理：24 小时前 Temp 临时文件、旧 Electron 解压残留、回收站清空。
...
- 支持应用图标、关闭时最小化到系统托盘、本地项目占用提醒和手动检查版本更新。
...
WinStatusInsight 默认偏保守：系统进程、系统级自启动项、安全软件、更新服务和高风险应用数据不会被静默处理。可操作项会在按钮文案、禁用状态和执行结果中明确说明，避免误删、误停或破坏系统配置。
